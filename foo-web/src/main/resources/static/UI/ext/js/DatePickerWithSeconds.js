...
Ext.define('Ext.ux.DateTimePicker', {
    ...
    renderTpl: [
        '<div id="{id}-innerEl" data-ref="innerEl" role="presentation">',
            ...
            '<tpl if="showToday">',
              ...
              '<div id="{id}-footerEl" role="presentation" style="background-color:#D9E5F3;border-top:0px solid #99BCE8;">{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}<center>&nbsp;{%this.renderTodayBtn(values, out)%}</center></div>',
              '<div style="text-align:center;margin-top:10px;">{%this.renderOkQueDingBtn(values, out)%}</div>',
            '</tpl>',
            // These elements are used with Assistive Technologies such as screen readers
            '<div id="{id}-todayText" class="' + Ext.baseCSSPrefix + 'hidden-clip" style="margin-top:20px">{todayText}.</div>',
    ...
    handleDateClick: function(e, t) {
        ...
        if(!me.disabled && t.dateValue && !Ext.fly(t.parentNode).hasCls(me.disabledCellCls)){
            me.setValue(new Date(t.dateValue));
            me.fireEvent('select', me, me.value); //关闭日历框 
            if (handler) {
                handler.call(me.scope || me, me, me.value);
            }
 ...
 *     });
 */
Ext.define('Ext.ux.DateFieldWithSeconds', {
