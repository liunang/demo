...
var NTGridBean = function (){
	...
	//默认操作按钮
	var linkBtnHtml = '<button class="btn btn-sm btn-success" style="background:#BBCA11;border:none;" onclick="linkBtn(this)" title="链接">' +
						'链接</button>';
	
	var viewBtnHtml = 	'<button class="btn btn-sm btn-success" style="background:#BBCA11;border:none;"  onclick="viewBtn(this)" title="详情">' +
							'详情</button>';
	
	var editBtnHtml = 	'<button class="btn btn-sm btn-primary" style="background:#F89F0A;border:none;" onclick="editBtn(this)" title="编辑">' +
							'编辑</button>';
	
	var removeBtnHtml = '<button class="btn btn-sm btn-danger" style="background:#FC6E50;border:none;" onclick="removeBtn(this)" title="删除">' +
							'删除</button>';
	...
	var getAllOperateBtns = function(){
		...
		if(defaultBtns.linkBtn!=null&&defaultBtns.linkBtn=="show"){
		    allOprateBtns.push({'btnName':'linkBtn','text':linkBtnHtml});
		}
	...
	var getTbodyStr = function(data){
		...
		$(data).each(function(index,ele){
			var tr = "<tr pk='"+ele[pk]+"' isPush='"+ele[isPush]+"' columnValue='"+ele[columnValue]+"'  style='border:solid red 1px'>";
			if(checked==true)
			...
			var allOprateBtns = getAllOperateBtns();
			var width = allOprateBtns.length*70
			var operateHtml = getAllOperateBtnsHtml();
			if(operateHtml!=null&&operateHtml!=""){
